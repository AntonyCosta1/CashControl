import { despesasAPI, auth } from './api.js';
...
function normalizarDespesa(e) {
  return {
    ...
    cat: e.cat ?? e.categoria ?? 'Outros',
    val: Number(e.val ?? e.valor ?? 0),
    date: e.date ?? e.data ?? '',
    pay: e.pay ?? e.pagamento ?? '',
...
async function carregarDespesas() {
  try {
    ...
    expenses = Array.isArray(data) ? data.map(normalizarDespesa) : [];
    renderDashboard();
  } catch (error) {
    console.error(error);
...
function fmtMonth(m) {
  const [y, mo] = m.split('-');
  return new Date(y, mo - 1).toLocaleString('pt-BR', {
    month: 'long',
    year: 'numeric',
  });
}
...
function renderReports() {
  ...
  const catBars = document.querySelector('.cat-bars');
  if (catBars) {
...
function handleExportPDF() {
  ...
  const expRows = [...expenses].sort((a, b) => String(b.date).localeCompare(String(a.date)))
    .map(e => `<tr><td>${e.desc}</td><td>${e.cat}</td><td>${fmt(e.val)}</td><td>${fmtDate(e.date)}</td><td>${e.pay}</td></tr>`)
    .join('');

  const catRows = Object.entries(catData).sort((a, b) => b[1] - a[1])
    .map(([cat, val]) => `<tr><td>${cat}</td><td>${fmt(val)}</td></tr>`)
    .join('');
  ...
  <p>Gerado em ${new Date().toLocaleDateString('pt-BR')} — Total: <strong>${fmt(total)}</strong></p>
